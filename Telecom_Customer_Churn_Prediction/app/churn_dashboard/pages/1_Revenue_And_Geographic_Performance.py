...
with c1:
    ...
    if len(city_g):
        fig = px.scatter_mapbox(
            city_g, lat="Latitude", lon="Longitude", size="Churn_Rate",
            color="Churn_Rate", color_continuous_scale=["#F6D9CE", ACCENT, "#7A3A35"],
            size_max=28, zoom=4.6, hover_name="City",
            hover_data={"Churn_Rate": True, "Latitude": False, "Longitude": False},
        )
        fig.update_layout(mapbox_style="carto-darkmatter",
                           mapbox_center=dict(lat=36.7, lon=-119.4))
