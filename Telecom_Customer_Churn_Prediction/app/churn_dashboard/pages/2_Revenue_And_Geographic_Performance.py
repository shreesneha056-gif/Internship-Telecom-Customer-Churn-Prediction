...
with st.sidebar:
    ...
    slicer_open("Offer_Type")
    payment_offer_filter = st.multiselect(
        "Offer_Type", OFFER_OPTIONS, default=OFFER_OPTIONS,
...
with c1:
    ...
    if not city_agg.empty:
        city_agg["churn_rate"] = (city_agg["churned"] / city_agg["total"] * 100)

        def tier_color(rate):
            if rate < 15:
                return "#044A2F"   # green — low churn
            elif rate < 30:
                return "#6D5A00"   # amber — medium churn
            return "#6B2328"       # red — high churn

        city_agg["color"] = city_agg["churn_rate"].apply(tier_color)

        fig = go.Figure(
            go.Scattergeo(
                lat=city_agg["lat"], lon=city_agg["lon"],
                text=[f"{c}: {r:.1f}% churn ({t} customers)" for c, r, t in
                      zip(city_agg.index, city_agg["churn_rate"], city_agg["total"])],
                marker=dict(
                    size=(city_agg["churn_rate"] / city_agg["churn_rate"].max() * 30 + 5),
                    color=city_agg["color"], opacity=0.75, line=dict(width=0),
                ),
                hoverinfo="text",
            )
        )
        ...
        legend_html = (
            '<span style="color:#044A2F">●</span> &lt;15% churn &nbsp;&nbsp;'
            '<span style="color:#6D5A00">●</span> 15\u201330% churn &nbsp;&nbsp;'
            '<span style="color:#6B2328">●</span> \u226530% churn'
        )
    ...
    st.caption(
        ...
        "California cities only. Bubble size and color both driven by that city's churn rate."
    )
    chart_card_close()
...
st.caption(
    "Computed live from the real Telco customer dataset. City and Offer_Type "
