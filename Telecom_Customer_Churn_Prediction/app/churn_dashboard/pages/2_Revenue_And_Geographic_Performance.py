...
with c1:
    ...
    if not map_df.empty:
        status_colors = {"Stayed": "#01B8AA", "Churned": "#FD625E", "Joined": "#F2C80F"}
        fig = go.Figure()
        for status, color in status_colors.items():
            sub = map_df[map_df["Customer Status"] == status]
            if sub.empty:
                continue
            fig.add_trace(
                go.Scattergeo(
                    ...
                    name=status,
                    marker=dict(size=5, color=color, opacity=0.65, line=dict(width=0)),
                    hoverinfo="skip",
                )
            )
        fig.update_geos(
        ...
        fig.update_layout(
            ...
            legend=dict(orientation="h", yanchor="bottom", y=1.02, x=0, font=dict(color=colors.WHITE)),
        )
    ...
    st.caption(
        ...
        "California cities only. Each dot is one customer, colored by Customer Status."
    )
    chart_card_close()
