...
with c1:
    ...
    fig = go.Figure(
        go.Scattergeo(
            ...
            marker=dict(
                size=d.city_bubbles["size"], color=d.ROSE, opacity=0.6,
                line=dict(width=0),
            ),
        )
    )
    fig.update_geos(
        projection_type="natural earth", bgcolor=d.BG,
        landcolor="#1F1F1F", showocean=True, oceancolor=d.BG,
        showcountries=True, countrycolor="#4A4A4A", showcoastlines=False,
    )
    fig.update_layout(
